...
import {
  useWLHandoffItems,
  useWLHandoffItemMutations,
} from '@/hooks/wl/useWLHandoffItems';
import { WLHandoffItemEditor } from './WLHandoffItemEditor';
...
export function WLHandoffItemsTab({ handoffId, templateKey, focusKey }: Props) {
  const { data: items, isLoading } = useWLHandoffItems(handoffId);
  const { seedFromTemplate } = useWLHandoffItemMutations(handoffId);
  ...
  useEffect(() => {
    ...
    const el = document.getElementById(`item-${focusKey}`);
      ...
      <Card className="p-8 text-center space-y-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Load the default intake fields for this onboarding type.
          </p>
        </div>
        <Button
          onClick={() => seedFromTemplate.mutate(templateKey ?? 'standard')}
          disabled={seedFromTemplate.isPending}
        >
          Load default intake items
        </Button>
      </Card>
    );
  }
