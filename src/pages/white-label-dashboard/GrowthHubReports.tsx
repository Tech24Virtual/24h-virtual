...
export default function GrowthHubReports() {
  const { user } = useAuth();
  ...
  const { data: report, isLoading: loadingReport } = useQuery({
    queryKey: ["wl-seo-report", partner?.id, selectedMonth],
    queryFn: async () => {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.functions.invoke("wl-generate-seo-report", {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["wl-seo-report", partner?.id, selectedMonth] });
  ...
  const downloadPDF = () => {
    ...
    const doc = new jsPDF();
    const companyName = partner?.company_name || "SEO Report";

    doc.setFontSize(20);
    doc.text(`${companyName} - SEO Report`, 20, 25);
    doc.setFontSize(12);
    ...
    doc.setFontSize(11);
    doc.text(`Posts Generated: ${stats.posts_generated || 0}`, 25, 60);
    doc.text(`Posts Published: ${stats.posts_published || 0}`, 25, 68);
    doc.text(`Keywords Tracked: ${stats.keywords_tracked || 0}`, 25, 76);
    doc.text(`Keywords with Content: ${stats.keywords_with_content || 0}`, 25, 84);
    doc.text(`Coverage Rate: ${stats.coverage_rate || 0}%`, 25, 92);
    ...
    doc.save(`${companyName.replace(/\s+/g, "-")}-SEO-Report-${selectedMonth}.pdf`);
  };

  const statCards = [
    { label: "Posts Generated", value: stats?.posts_generated ?? "—", icon: FileText },
    { label: "Posts Published", value: stats?.posts_published ?? "—", icon: TrendingUp },
    { label: "Keywords Tracked", value: stats?.keywords_tracked ?? "—", icon: Search },
    { label: "Coverage Rate", value: stats ? `${stats.coverage_rate}%` : "—", icon: BarChart3 },
  ];
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Generate reports to share with your clients.</p>
          </div>
        </div>
