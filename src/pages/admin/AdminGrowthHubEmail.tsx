...
export default function AdminGrowthHubEmail() {
  ...
  const { data: connection } = useQuery({
    queryKey: ["admin-email-connection"],
    queryFn: async () => {
      const { data } = await supabase.from("admin_email_connections").select("*").limit(1).maybeSingle();
  ...
  const { data: contacts } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("admin_email_contacts").select("*").eq("subscribed", true).order("created_at", { ascending: false });
      return data || [];
    },
  });
  ...
  const { data: sends } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("admin_email_sends").select("*").order("created_at", { ascending: false }).limit(10);
      return data || [];
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = { provider: "resend" as const, api_key_encrypted: apiKey, from_name: fromName, from_email: fromEmail, status: "connected", last_tested_at: new Date().toISOString() };
      if (connection) {
        const { error } = await supabase.from("admin_email_connections").update(payload).eq("id", connection.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("admin_email_connections").insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-email-connection"] });
  ...
  const addContactMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("admin_email_contacts").insert({ email: newContactEmail, name: newContactName || null });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteContactMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("admin_email_contacts").update({ subscribed: false }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin-email-contacts"] }),
  });
        ...
        <TabsContent value="connection" className="space-y-4 mt-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2"><Label>From Name</Label><Input placeholder="24H Virtual" value={fromName} onChange={(e) => setFromName(e.target.value)} /></div>
