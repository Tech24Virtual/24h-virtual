...
export default function AdminGrowthHubNewsletter() {
  ...
  const { data: drafts, isLoading } = useQuery({
    queryKey: ["admin-newsletter-drafts", selectedMonth],
    queryFn: async () => {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.functions.invoke("admin-generate-newsletter", {
        body: { draft_month: selectedMonth },
      });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-newsletter-drafts", selectedMonth] });
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      if (!latestDraft) throw new Error("No draft to send");
      const { data, error } = await supabase.functions.invoke("wl-send-newsletter", {
        body: {
          scope: "admin",
          newsletter_draft_id: latestDraft.id,
          subject: latestDraft.subject_line,
          html_content: latestDraft.html_content,
        },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Auto-draft email newsletters from your blog posts.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a month to create a newsletter from that month's published blog posts.</CardDescription>
        </CardHeader>
      ...
      {latestDraft && (
        <>
          ...
          <Card>
            ...
            <CardContent>
              {viewMode === "preview" ? (
                <div className="border rounded-lg p-4 bg-background" dangerouslySetInnerHTML={{ __html: latestDraft.html_content || "" }} />
              ) : (
                <pre className="text-xs bg-muted p-4 rounded-lg overflow-auto max-h-96">{latestDraft.html_content}</pre>
              )}
            </CardContent>
          </Card>
