...
// Using CalendarMeeting type from shared component, extended with notes for this page
...
export default function SalesMeetings() {
  ...
  const filteredByTab = meetings.filter(m => {
    const d = new Date(m.scheduled_at);
    if (activeTab === 'upcoming') return d >= todayStart && m.status !== 'cancelled';
    if (activeTab === 'past') return d < todayStart || m.status === 'completed' || m.status === 'no_show';
    return true; // all
  });
    ...
    <StaffLayout role="sales">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Track and manage scheduled meetings</p>
        </div>
