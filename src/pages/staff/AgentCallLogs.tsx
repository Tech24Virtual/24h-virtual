...
export default function AgentCallLogs() {
  ...
  const { data: relatedCalls = [], isFetching: relatedLoading } = useQuery({
    queryKey: ['call-logs-related', selectedCall?.caller_phone],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('call_logs')
  ...
  const handleOpenCall = (call: CallLog) => {
    ...
    setAgentNotesDraft(call.agent_notes || '');
    setNoteSaved(false);
  };
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <DetailRow
                    label="Handle Time"
                    value={formatDuration(selectedCall.handle_time_seconds)}
                  />
                ...
                {/* Call notes from Five9 (read-only) */}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={agentNotesDraft}
                    onChange={(e) => {
                      setAgentNotesDraft(e.target.value);
                  ...
                  <div className="flex justify-end">
                    <Button
                      size="sm"
                      onClick={() => saveNotesMutation.mutate({ id: selectedCall.id, notes: agentNotesDraft })}
                      disabled={saveNotesMutation.isPending || (!noteSaved && agentNotesDraft === (selectedCall.agent_notes || ''))}
