...
export default function BillingTickets() {
  const [dialogOpen, setDialogOpen] = useState(false);
  const queryClient = useQueryClient();
  
  useRealtimeTickets({ workQueueFilter: 'billing' });
    ...
    <StaffLayout role="billing">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Billing Tickets</h1>
            <p className="text-muted-foreground">
              Manage billing-related support requests
            </p>
          </div>
        ...
        <Tabs defaultValue="department">
          ...
          <TabsContent value="created">
            <CreatedTicketsList excludeSource="billing" />
          </TabsContent>
        </Tabs>
      </div>
